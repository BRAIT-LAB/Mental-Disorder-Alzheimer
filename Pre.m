clear ;clc;
path = ['F:\sleepdata\set'];           %路径下放有需要分析的所有.set文件
cd(path);
AllFile = dir('**/*.*set');
...
EEG = pop_chanedit(EEG, 'lookup','D:\\Matlab\\toolbox\\eeglab_current\\plugins\\dipfit\\standard_BESA\\standard-10-5-cap385.elp');
EEG = eeg_checkset( EEG );
EEG = pop_reref( EEG, []);
EEG = eeg_checkset( EEG );
